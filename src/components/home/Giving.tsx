              ...
              <Reveal key={tier.amount} delay={i * 70} amount={0.08}>
                ...
                >
                  ...
                  <p className="flex items-baseline gap-1.5">
                    ...
                    <span
                      className={`font-display text-[2.1rem] leading-none font-black transition-transform duration-300 origin-left group-hover:scale-105 ${styling.text}`}
                    >
                      {tier.amount}
                    </span>
                  </p>

                  <h3 className="mt-4 font-display text-[1.12rem] font-extrabold text-[#0A1020] transition-colors group-hover:text-[#075BD6]">
                    {tier.title}
                  </h3>
                  <p className="mt-2.5 flex-1 text-[0.88rem] leading-relaxed text-[#4B5563]">
                    {tier.body}
                  </p>

                  <Link
                    href="/donate"
                    className="mt-6 flex items-center justify-between rounded-xl bg-[#F5F7FA] border border-[#DCE2EA] px-4 py-2.5 font-display text-[0.85rem] font-bold text-[#0A1020] transition-all duration-200 group-hover:bg-[#075BD6] group-hover:border-[#075BD6] group-hover:text-white"
                  >
        ...
        <Reveal delay={80}>
          <div className="mt-9 grid gap-7 rounded-2xl border border-[#DCE6FB] bg-gradient-to-br from-[#F3F7FF] via-[#F8FAFC] to-white p-6 sm:p-7 shadow-[0_4px_20px_rgba(6,73,184,0.06)] lg:grid-cols-12 lg:items-center lg:p-8">
            <div className="lg:col-span-5">
              <span className="inline-flex items-center gap-1.5 font-display text-[0.78rem] font-extrabold uppercase tracking-wider text-[#075BD6]">
              ...
              <h3 className="mt-2 font-display text-[1.32rem] font-extrabold text-[#0A1020]">
